...
$BINARY tx channel test1-test2 --home $CHAIN_DIR/relayer --version ics20-1
#$BINARY tx channel test1-test2 --home $CHAIN_DIR/relayer --src-port transfer --dst-port transfer --order unordered 

echo "Starting to listen relayer..."
$BINARY start test1-test2 -p events -b 100 --home $CHAIN_DIR/relayer > $CHAIN_DIR/relayer.log 2>&1 &
