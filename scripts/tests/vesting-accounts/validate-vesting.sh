...
CREATE_VESTING_ACCOUNT_MSG_RES=$($BINARY tx vesting create-periodic-vesting-account $RANDOM_VESTING_WALLET $HIDDEN_VESTING_FILE --from $WALLET_3 --chain-id test-1 --home $CHAIN_DIR/test-1 --node tcp://localhost:16657  --keyring-backend test -y -o json | jq -r '.logs[0].events[2].attributes[0].value')
