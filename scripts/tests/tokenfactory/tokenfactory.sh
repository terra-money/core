...
CREATED_RES_DENOM=$($BINARY tx tokenfactory create-denom $TOKEN_DENOM --from $WALLET_1 --home $CHAIN_DIR/test-1 --chain-id test-1 --node tcp://localhost:16657 --broadcast-mode block --keyring-backend test -o json -y | jq -r '.logs[0].events[2].attributes[1].value')
...
MINT_RES=$($BINARY tx tokenfactory mint $MINT_AMOUNT$CREATED_RES_DENOM --from $WALLET_1 --home $CHAIN_DIR/test-1 --chain-id test-1 --node tcp://localhost:16657 --broadcast-mode block --keyring-backend test -o json -y | jq -r '.logs[0].events[2].type')
...
BURN_RES=$($BINARY tx tokenfactory burn 1$CREATED_RES_DENOM --from $WALLET_1 --home $CHAIN_DIR/test-1 --chain-id test-1 --node tcp://localhost:16657 --broadcast-mode block --keyring-backend test -o json -y | jq -r '.logs[0].events[4].type')
...
SEND_RES_MSG_TYPE=$($BINARY tx bank send $WALLET_1 $WALLET_3 1$CREATED_RES_DENOM --from $WALLET_1 --home $CHAIN_DIR/test-1 --chain-id test-1 --node tcp://localhost:16657 --broadcast-mode block --keyring-backend test -o json -y | jq -r '.logs[0].events[2].attributes[0].value')
...
IBC_SEND_RES=$($BINARY tx ibc-transfer transfer transfer channel-0 $WALLET_2 1$CREATED_RES_DENOM --chain-id test-1 --home $CHAIN_DIR/test-1 --node tcp://localhost:16657 --keyring-backend test --from $WALLET_1 --broadcast-mode block -y -o json | jq -r '.logs[0].events[3].attributes[0].value')
